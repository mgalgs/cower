...
the 2 character minimum and lack of regex that the AUR offers. Cower will
search based on the first span of 2 non-regex characters it finds and filter
using the regular expression provided. cower makes no guarantees that complex
patterns will return accurate results.
