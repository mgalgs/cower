...
patterns as defined by POSIX are allowed.
...
=item B<--ignorerepo=>I<REPO>

Ignore a binary repo when checking for updates. Can be used more than once.
Also accepts a comma delimited list as a single argument.

=item B<--listdelim=>I<STRING>

Specify a delimiter when printing list formatters. This option only has an
effect when using the -ii operation combined with --format. See the FORMATTING
section.
...
the PKGBUILD specifies them). Items in lists are delimited according to the
string specified by the --listdelim option, defaulting to 2 spaces.
