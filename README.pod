...
patterns (except for character ranges) as defined by POSIX are allowed.

B<Note:> This is more or less a hack because it's implemented on top of what
the 2 character minimum and lack of regex that the AUR offers. Cower will search
based on the first span of 2 non-regex characters it finds and filter using the
...
When the B<--format> option is used the following sequences are interpreted
